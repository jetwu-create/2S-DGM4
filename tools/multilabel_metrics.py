...
def get_multi_label(label, image):
    ...
    FS_pos = np.where(np.array(label) == 'face_swap')[0].tolist()
    FA_pos = np.where(np.array(label) == 'face_attribute')[0].tolist()
    TS_pos = np.where(np.array(label) == 'text_swap')[0].tolist()
    TA_pos = np.where(np.array(label) == 'text_attribute')[0].tolist()
    FSTS_pos = np.where(np.array(label) == 'face_swap&text_swap')[0].tolist()
    FSTA_pos = np.where(np.array(label) == 'face_swap&text_attribute')[0].tolist()
    FATS_pos = np.where(np.array(label) == 'face_attribute&text_swap')[0].tolist()
    FATA_pos = np.where(np.array(label) == 'face_attribute&text_attribute')[0].tolist()

    return real_label_pos, FS_pos, FA_pos, TS_pos, TA_pos, FSTS_pos, FSTA_pos, FATS_pos, FATA_pos
